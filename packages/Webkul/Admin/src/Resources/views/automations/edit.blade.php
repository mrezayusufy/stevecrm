...
@section('page_title')
    {{ __('admin::app.automations.edit-title') }}
@stop
@section('title')
    {{ __('admin::app.automations.edit-title') }}
@stop
...
@section('content-wrapper')
    <div class="content full-page adjacent-center">
        ...
        <form method="POST" action="{{ route('admin.automations.update', $automation->id) }}" @submit.prevent="onSubmit">
                ...
                <div class="form-container">
                    ...
                    <div class="panel">
                        <div class="panel-header">
                            {!! view_render_event('admin.automations.edit.form_buttons.before', ['automation' => $automation]) !!}

                            <button type="submit" class="btn btn-md btn-primary">
                                {{ __('admin::app.automations.save-btn-title') }}
                            </button>

                            <a href="{{ route('admin.automations.index') }}">{{ __('admin::app.automations.back') }}</a>

                            {!! view_render_event('admin.automations.edit.form_buttons.after', ['automation' => $automation]) !!}
                        </div>
        
                        <div class="panel-body">
                            {!! view_render_event('admin.automations.edit.form_controls.before', ['automation' => $automation]) !!}
                            ...
                            <div class="form-group" :class="[errors.has('title') ? 'has-error' : '']">
                                <label for="comment" class="required">{{ __('admin::app.automations.title-control') }}</label>
                            ...
                            <div class="form-group" :class="[errors.has('type') ? 'has-error' : '']">
                                <label for="type" class="required">{{ __('admin::app.automations.type') }}</label>
                            ...
                            <div class="form-group date" :class="[errors.has('schedule_from') || errors.has('schedule_to') ? 'has-error' : '']">
                                <label for="schedule_from" class="required">{{ __('admin::app.automations.schedule') }}</label>
                                ...
                                <div class="input-group">
                                    <datetime>
                                        <input
                                            ...
                                            placeholder="{{ __('admin::app.automations.from') }}" ref="schedule_from"
                                    ...
                                    <datetime>
                                        <input
                                            ...
                                            placeholder="{{ __('admin::app.automations.to') }}" ref="schedule_to"
                            ...
                            <div class="form-group">
                                <label for="location">{{ __('admin::app.automations.location') }}</label>
                            ...
                            <div class="form-group">
                                <label for="comment">{{ __('admin::app.automations.description') }}</label>
                                <textarea class="control" id="automation-comment" name="comment">{{ old('comment') ?: $automation->comment }}</textarea>
                            </div>
        
                            <div class="form-group">
                                <label for="participants">{{ __('admin::app.automations.participants') }}</label>
        
                                <multi-lookup-component :data='@json($automation->participants)'></multi-lookup-component>
                            </div>

                            <div class="form-group">
                                <label for="validation">{{ __('admin::app.automations.lead') }}</label>
                            ...
                            {!! view_render_event('admin.automations.edit.form_controls.after', ['automation' => $automation]) !!}
    ...
    <script type="text/x-template" id="multi-lookup-component-template">
        <div class="lookup-control">
            <div class="form-group" style="margin-bottom: 0">
                <input type="text" class="control" v-model="search_term" v-on:keyup="search" autocomplete="off" placeholder="{{ __('admin::app.automations.typing-placeholder') }}">
        ...
        Vue.component('multi-lookup-component', {
            ...
            methods: {
                search: debounce(function () {
                    ...
                    this.$http.get("{{ route('admin.automations.search_participants') }}", {params: {query: this.search_term}})
                        .then (response => {
                            var self = this;
