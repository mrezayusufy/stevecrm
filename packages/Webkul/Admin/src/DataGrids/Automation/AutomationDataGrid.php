...
namespace Webkul\Admin\DataGrids\Automation;
...
class AutomationDataGrid extends DataGrid
{
    ...
    {
        $queryBuilder = DB::table('automations')
            ->distinct()
            ->select(
                'automations.*',
                'leads.id as lead_id',
            ...
            ->leftJoin('automation_participants', 'automations.id', '=', 'automation_participants.automation_id')
            ->leftJoin('lead_automations', 'automations.id', '=', 'lead_automations.automation_id')
            ->leftJoin('leads', 'lead_automations.lead_id', '=', 'leads.id')
            ...
            if ($currentUser->view_permission == 'group') {
                $queryBuilder->where(function ($query) use ($currentUser) {
                    ...
                    $query->whereIn('automations.user_id', $userIds)
                        ->orWhereIn('automation_participants.user_id', $userIds);
                ...
                $queryBuilder->where(function ($query) use ($currentUser) {
                    $query->where('automations.user_id', $currentUser->id)
                        ->orWhere('automation_participants.user_id', $currentUser->id);
        ...
        $this->addFilter('id', 'automations.id');
        $this->addFilter('title', 'automations.title');
        $this->addFilter('schedule_from', 'automations.schedule_from');
        $this->addFilter('created_by', 'users.name');
        $this->addFilter('created_by_id', 'automations.user_id');
        $this->addFilter('created_at', 'automations.created_at');
    ...
    public function addColumns()
    {
        $this->addColumn([
            ...
            'closure'          => function ($row) {
                return view('admin::automations.datagrid.is-done', compact('row'))->render();
            },
        ]);
    ...
    public function prepareActions()
    {
        ...
        $this->addAction([
            ...
            'method'       => 'DELETE',
            'route'        => 'admin.automations.delete',
    ...
    public function prepareMassActions()
    {
        $this->addMassAction([
            ...
            'action'  => route('admin.automations.mass_update'),
            'method'  => 'PUT',
            'options' => [
        ...
        $this->addMassAction([
            ...
            'action' => route('admin.automations.mass_delete'),
            'method' => 'PUT',
        ]);
    }
}
